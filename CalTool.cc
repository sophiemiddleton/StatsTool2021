...
struct results_struct {
    ...
    double N_CE_gen;
    double N_CE_rec;
    double efficiency_CE;
    double efficiency_error_CE;
    double N_CE_expected;
    double N_CE_expected_error;
    double SES;
    double SES_error;
    double N_DIO_gen;
    double N_DIO_gen_error;
    double N_DIO_rec;
    double N_DIO_rec_error;
    double efficiency_DIO;
    double efficiency_error_DIO;
    double N_DIO_expected;
    double N_DIO_expected_error;
    double N_RPCs_expected;
    double N_RPCs_expected_error;
    double Nsig_UL;
    double Nsig_UL_error;
    double BF_UL;
    double BF_UL_error;
    int optimal_window;
};
...
/*
defintions for histograms
 */
//double momentum_lower_limit=102.5;
//double momentum_upper_limit=107.;
double momentum_lower_limit=90.;
double momentum_upper_limit=110.;
//int nBins=90;
int nBins=400;
//int nBin_steps=(momentum_upper_limit-momentum_lower_limit)/nBins;
double momentum_Bin_width=0.05; // 50keV bin width

int flag_DIO_reweighting;

TH1F *histo_RPCs;
TH1F *histo_CE_generated;
TH1F *histo_CE_reconstructed;
TH1F *histo_DIO_generated_flat;
TH1F *histo_DIO_reconstructed_flat;
TH1F *histo_DIO_generated_reweighted;
TH1F *histo_DIO_reconstructed_reweighted;

TH1F *histo_DIO_reconstructed_reweighted_rescaled_to_final_luminosity;
TH1F *histo_total_background_DIO_RPC_rescaled_to_final_luminosity; // DIO + RPC background

TF1 *_diocz_f;

Float_t GetValue_Branch_Leaf_Version(TTree *t, std::string branch_name, std::string leaf_name) {
    TBranch *temp_branch;
    TLeaf *temp_leaf;
    ...
    if (objarray->GetEntries() == 0) {
        std::cout << "Can't find any branch in tree " << t->GetName() << ". Aborting." << std::endl;
        abort();
    }

//    temp_branch = t->GetBranch("h1");
//    temp_branch = t->GetBranch(name.c_str());
//    temp_branch = t->GetBranch("fit");
    temp_branch = t->GetBranch(branch_name.c_str());

    temp_leaf = temp_branch->FindLeaf(leaf_name.c_str());

    for (Int_t i = 0; i < objarray->GetEntries(); i++) {
        temp_branch = (TBranch*) objarray->At(i);

        temp_leaf = temp_branch->FindLeaf(leaf_name.c_str());

        if (temp_leaf != 0) {
            temp_value = temp_leaf->GetValue();
            return temp_value;
        }
    }
    std::cout << "Didn't find branch " << branch_name << " leaf " << leaf_name << " in tree " << t->GetName() << ". Aborting." << std::endl;
    abort();
    return -999;
}

/*
Function to calculate and integrate the DIO spectrum
*/
...
/*
Function to calculate DIO weights to reweight flat DIO spectra
*/
Double_t calculate_DIOweight(Double_t x) {
  double ee = x;
  static const double mal(25133);
  //    double mmu(105.654);
  static const double emu(105.194);
...
*/
  ...
  double FeldmanCousins_sensitivityValues[2001] =
  {
 ...
 */
 double return_FeldmanCousins_sensitivity(double background_mean) {
	  if (background_mean<10){ // have only values up to 10 background events in table, will not work for wide DIO spectrum
		  int    nb        = 2001;
		  double bInterval = 0.005;
		  int background_mean_rounded_array_index = TMath::Nint(background_mean / bInterval);

		  //if (background_mean_rounded_array_index<nb) {
		  if (background_mean_rounded_array_index>0) {
			  if (background_mean_rounded_array_index<nb) {
				  std::cout << "background_mean_rounded_array_index == " << background_mean_rounded_array_index << std::endl;
...
double return_histogram_integral(TH1F *histo, double mom_low, double mom_high)
{
	//cout << "momentum_lower_limit = " << momentum_lower_limit << endl;
	//cout << "momentum_upper_limit = " << momentum_upper_limit << endl;
	//cout << "momentum_Bin_width   = " << momentum_Bin_width << endl;

	// translate mom_low and mom_up in bin numbers
	int bin_low = TMath::Nint((mom_low - momentum_lower_limit) / momentum_Bin_width) + 1;
	int bin_high = TMath::Nint((mom_high - momentum_lower_limit) / momentum_Bin_width) + 1;

	//cout << "mom_low  = " << mom_low << "   bin_low  = " << bin_low << endl;
	//cout << "mom_high = " << mom_high << "  bin_high = " << bin_high << endl;

	return histo->Integral(bin_low,bin_high);
}

double calc_Nrec(TH1F *histo, double mom_low, double mom_high)
{
...
//	double Nrec_error = 0;
	...
	if ( fabs(mom_low-103.5)<0.01 && fabs(mom_high-105.5)<0.01) {
		cout << "Nrec = " << Nrec << endl;
	}
...
//	}
//	Nrec_error = sqrt(temp_error_sum);

	return Nrec;
}

double calc_Nrec_error(TH1F *histo, double mom_low, double mom_high)
{
	double Nrec_error = 0;
...
double calc_efficiency(double Nrec, double Ngen)
{
...
double calc_efficiency_error(double Nrec, double Ngen)
{
	...
	double efficiency_error = sqrt(Nrec * (1. - Nrec/Ngen)) / Ngen;

	// cout << "efficiency_error = " << efficiency_error << endl;

	return efficiency_error;
}

double calc_efficiency_error_DIO(double Nrec, double Nrec_error, double Ngen, double Ngen_error)
{
	// use error propagation on eff = N_rec / N_gen
//	double Nrec_error = sqrt(Nrec);
//	double Ngen_error = sqrt(Ngen);

	double efficiency_error = sqrt( pow(Nrec_error / Ngen, 2) + pow( Nrec * Ngen_error / (Ngen*Ngen), 2) );
...
double calc_N_CE_expected(double POT, double stopsperPOT, double capturesperStop, double efficiency_CE)
{
	// assume BF of 10E-16 and calculate expected signal for this BF
	double BF_assumption=1E-16;

	double N_CE_expected = POT * stopsperPOT * capturesperStop * BF_assumption * efficiency_CE;

	//cout << "N_CE_expected = " << N_CE_expected << endl;

	return N_CE_expected;
}

double calc_SES(double POT, double stopsperPOT, double capturesperStop, double efficiency_CE)
{
	// calculate single event sensitivity (SES), corresponds to branching fraction where 1 signal event is observed
	double SES = 1. / ( POT * stopsperPOT * capturesperStop * efficiency_CE );
...
double calc_SES_error(double POT, double stopsperPOT, double capturesperStop, double efficiency_CE, double efficiency_error_CE)
{
...
/*
Integrate Czarnecki spectrum and normalize number of generated DIOs
*/
//TF1 *_diocz_f = new TF1("_diocz_f",DIOCZ,95.0,107,1); // global function to speed up integral calculation
//TF1 *_diocz_f = new TF1("_diocz_f",DIOCZ,90.,107,1); // global function to speed up integral calculation
//
//double calc_Ngen_DIO(double mom_low, double mom_high, double Ngen_DIO)
//{
//	//_diocz_f = new TF1("_diocz_f",DIOCZ,95.0,trueconvmom,1);
//	//TF1 *_diocz_f = new TF1("_diocz_f",DIOCZ,95.0,107,1);
...
//	double integral_DIO = _diocz_f->Integral(mom_low,mom_high);
//
//	double integral_DIO_normalisation;
//
//	if (flag_DIO_reweighting==1) {
////		integral_DIO_normalisation = _diocz_f->Integral(95,107.);
//		integral_DIO_normalisation = _diocz_f->Integral(101.5,107.);
//	} else {
//		integral_DIO_normalisation = _diocz_f->Integral(101.5,107.);
//	}
//	//double integral_DIO_normalisation = _diocz_f->Integral(102.5,107.);
//
//	double Ngen_DIO_thismomentumrange = Ngen_DIO * integral_DIO / integral_DIO_normalisation;
//
//	//cout << "integral_DIO = " << integral_DIO << endl;
//	//cout << "integral_DIO_normalisation = " << integral_DIO_normalisation << endl;
//	//cout << "Ngen_DIO_thismomentumrange = " << Ngen_DIO_thismomentumrange << endl;
//
//	/*
//	if ( fabs(mom_low-103.75)<0.01 && fabs(mom_high-105.45)<0.01) {
//		cout << "===========================================================================" << endl;
//		cout << "calc_Ngen_DIO(double mom_low, double mom_high, double Ngen_DIO):" << endl;
//		cout << "Ngen_DIO = " << Ngen_DIO << endl;
//		cout << "integral_DIO = " << integral_DIO << endl;
//		cout << "integral_DIO_normalisation = " << integral_DIO_normalisation << endl;
//		cout << "Ngen_DIO_thismomentumrange = " << Ngen_DIO_thismomentumrange << endl;
//	}
//	*/
//
//	return Ngen_DIO_thismomentumrange;
//}

//double calc_Ngen_DIO(TH1F *histo, double mom_low, double mom_high)
//{
//	double Ngen_DIO = return_histogram_integral(histo, mom_low, mom_high);
//
//	//cout << "Nrec = " << Nrec << endl;
//
//	if ( fabs(mom_low-103.5)<0.01 && fabs(mom_high-105.5)<0.01) {
...
double calc_N_DIO_expected(double N_DIO_rec, double N_DIO_gen, double POT, double stopsperPOT, double decaysperStop, double mom_low, double mom_high)
{
	double G_number_DIOs_represented = _diocz_f->Integral(mom_low,mom_high);

	double N_DIO_expected = N_DIO_rec * G_number_DIOs_represented * POT * stopsperPOT * decaysperStop / N_DIO_gen;

	//cout << "G_number_DIOs_represented = " << G_number_DIOs_represented << endl;
	//cout << "N_DIO_expected = " << N_DIO_expected << endl;

	if ( fabs(mom_low-103.75)<0.01 && fabs(mom_high-105.45)<0.01) {
		cout << "===========================================================================" << endl;
		cout << "calc_N_DIO_expected(double N_DIO_rec, double N_DIO_gen, double POT, double stopsperPOT, double decaysperStop, double mom_low, double mom_high):" << endl;
		cout << "N_DIO_rec = " << N_DIO_rec << endl;
		cout << "N_DIO_gen = " << N_DIO_gen << endl;
		cout << "POT = " << POT << endl;
		cout << "stopsperPOT = " << stopsperPOT << endl;
		cout << "decaysperStop = " << decaysperStop << endl;
		cout << "G_number_DIOs_represented = " << G_number_DIOs_represented << std::endl;
		cout << "N_DIO_expected = " << N_DIO_expected << endl;
	}
...
double calc_N_DIO_expected_error(double efficiency_error_DIO, double POT, double stopsperPOT, double decaysperStop, double mom_low, double mom_high)
{
	double G_number_DIOs_represented = _diocz_f->Integral(mom_low,mom_high);

	double N_DIO_expected_error = G_number_DIOs_represented * POT * stopsperPOT * decaysperStop * efficiency_error_DIO; // compute error on N_DIO_expected from error on the efficiency
	...
	if ( fabs(mom_low-103.75)<0.01 && fabs(mom_high-105.45)<0.01) {
		...
		cout << "decaysperStop = " << decaysperStop << endl;
		cout << "G_number_DIOs_represented = " << G_number_DIOs_represented << std::endl;
		cout << "N_DIO_expected_error = " << N_DIO_expected_error << endl;
	}

	return N_DIO_expected_error;
}

void calc_N_RPC_expected(double &integral_RPCs, double &integral_RPCs_error, double mom_low, double mom_high)
{
	integral_RPCs_error = 0;

//	double integral_RPCs = histo_RPCs->Integral( histo_RPCs->FindBin(mom_low), histo_RPCs->FindBin(mom_high) );
	integral_RPCs = histo_RPCs->IntegralAndError( histo_RPCs->FindBin(mom_low), histo_RPCs->FindBin(mom_high), integral_RPCs_error );

//	std::cout << "integral_RPCs == " << integral_RPCs << std::endl;
//	std::cout << "integral_RPCs_error == " << integral_RPCs_error << std::endl;

//	return integral_RPCs;
}

double calc_BF_upper_limit(double Nsig_UL, double POT, double stopsperPOT, double capturesperStop, double efficiency_CE)
{
	double BF_upper_limit = Nsig_UL / ( POT * stopsperPOT * capturesperStop * efficiency_CE );

	//cout << "BF_upper_limit = " << BF_upper_limit << endl;
...
double calc_BF_upper_limit_error(double Nsig_UL, double Nsig_UL_error, double POT, double stopsperPOT, double capturesperStop, double efficiency_CE, double efficiency_error_CE)
{
	// just use uncertainty on Nsig_UL
//	double BF_upper_limit_error = Nsig_UL_error / ( POT * stopsperPOT * capturesperStop * efficiency_CE );

	// just use uncertainty on efficiency_CE
//	double BF_upper_limit_error = Nsig_UL_error / ( POT * stopsperPOT * capturesperStop * efficiency_CE * efficiency_CE ) * efficiency_error_CE;

	// calculate error on the branching fraction by error propagation of the errors on Nsig_UL and on efficiency_CE
	double BF_upper_limit_error = 1. / (POT * stopsperPOT * capturesperStop) * sqrt( pow(Nsig_UL_error/efficiency_CE, 2) + pow(Nsig_UL * efficiency_error_CE/(efficiency_CE*efficiency_CE), 2));

	//cout << "BF_upper_limit_error = " << BF_upper_limit_error << endl;
...
int main(int argc, char* argv[]) {
	if (argc < 6) {
		fprintf(stderr, "usage: CalTool <target_mass_scalefactor> <CE generated inputfile> <CE reconstructed inputfile> <DIO generated inputfile> <DIO reconstructed inputfile>\n\n");
		fprintf(stderr, "<target_mass_scalefactor>: 1 corresponds to the nominal 162.22g target\n");
		fprintf(stderr, "                           0 corresponds to no use of RPCs at all\n");

		exit(3);
	}

	// init function for Czarnecki spectrum
	_diocz_f = new TF1("_diocz_f",DIOCZ,momentum_lower_limit,momentum_upper_limit,1); // global function to speed up integral calculation
	_diocz_f->SetLineColor(kGreen);
	_diocz_f->SetParameter(0,1.0);

	flag_DIO_reweighting = 1.;
	double Ngen_CE = 0;
	double Ngen_DIO = 0;

	double target_mass_scalefactor = boost::lexical_cast<double > (argv[1]);
	std::string inputfilename_CE_generated = argv[2];
	std::string inputfilename_CE_reconstructed = argv[3];
	std::string inputfilename_DIO_generated = argv[4];
	std::string inputfilename_DIO_reconstructed = argv[5];

	std::cout << "target_mass_scalefactor" << target_mass_scalefactor << std::endl;

	// create histograms
	histo_CE_reconstructed = new TH1F("histo_CE_reconstructed","histo_CE_reconstructed",nBins,momentum_lower_limit,momentum_upper_limit);
	histo_CE_generated = new TH1F("histo_CE_generated","histo_CE_generated",nBins,momentum_lower_limit,momentum_upper_limit);
	histo_DIO_reconstructed_flat = new TH1F("histo_DIO_reconstructed_flat","histo_DIO_reconstructed_flat",nBins,momentum_lower_limit,momentum_upper_limit);
	histo_DIO_generated_flat = new TH1F("histo_DIO_generated_flat","histo_DIO_generated_flat",nBins,momentum_lower_limit,momentum_upper_limit);
	histo_DIO_reconstructed_reweighted = new TH1F("histo_DIO_reconstructed_reweighted","histo_DIO_reconstructed_reweighted",nBins,momentum_lower_limit,momentum_upper_limit);
	histo_DIO_generated_reweighted = new TH1F("histo_DIO_generated_reweighted","histo_DIO_generated_reweighted",nBins,momentum_lower_limit,momentum_upper_limit);

	histo_CE_reconstructed->Sumw2();
	histo_CE_generated->Sumw2();
	histo_DIO_reconstructed_flat->Sumw2();
	histo_DIO_generated_flat->Sumw2();
	histo_DIO_reconstructed_reweighted->Sumw2();
	histo_DIO_generated_reweighted->Sumw2();

	//	TCut cuts_quality(" status==1 && nactive>=25 && con>=2.0e-3 && momerr<=0.25 && t0err<=0.9 && d0>=-80 && d0<=105 && (d0+2/om)>450 && (d0+2/om)<680 && t0>=710 && t0<=1695 && td>=0.57735 && td<=1. && 102.5<mom && mom<106 "); // MR20150727 cut set C

	//	// definitions for calculations
	//	double POT = 3.6E20;
	//	// double numberStopsDefault = 150940; // 75 TDR beam files
	//	// ouble numberStopsThis = Ngen_CE;
	//
	//	//double stopsperPOT = 0.0016 * numberStopsThis / numberStopsDefault;
	//	double stopsperPOT = numberStopsThis / (75. * 100. * 10000.);
	//
	//	double capturesperStop = 0.609;
	//	double decaysperStop = 0.391;


	////////////////////////////////////////////////////////////
	// read in RPC histogram
	////////////////////////////////////////////////////////////

	std::cout << std::endl << "Reading RPC histogram" << std::endl;

	TFile *file_RPCs = new TFile("./Mu2e_fit_RPCs_results.root", "READ");
	// TH1F *histo_RPCs = (TH1F*) file_RPCs->Get("histo_momentum_fitted");
	histo_RPCs = (TH1F*) file_RPCs->Get("histo_momentum_fitted");
	histo_RPCs->Sumw2();

	// scale RPC histogram to target mass
	histo_RPCs->Scale(target_mass_scalefactor);

	////////////////////////////////////////////////////////////
	// read in generated CE data and fill histogram
	////////////////////////////////////////////////////////////

	std::cout << std::endl << "Reading generated CE data" << std::endl;

	TFile *file_CE_generated = new TFile(inputfilename_CE_generated.c_str(), "READ");
	TTree *tree_CE_generated = (TTree*) file_CE_generated->Get("TrkAnaNeg/trkana");

	// loop over CE tree and fill histogram
	TBranch *b_fit_CE_generated = tree_CE_generated->GetBranch("demcgen");
	//TLeaf *l_fit_mom_CE_generated = b_fit_CE_generated->GetLeaf("mom");
  TLeaf *ce_mom_gen_x = b_fit_CE_generated->GetLeaf("momx");
  TLeaf *ce_mom_gen_y = b_fit_CE_generated->GetLeaf("momy");
  TLeaf *ce_mom_gen_z = b_fit_CE_generated->GetLeaf("momz");
	for( Long64_t i = 0; tree_CE_generated->GetEntry(i) > 0; i++ ) {
		Float_t fit_mom_CE_generated = sqrt(ce_mom_gen_x->GetValue()*ce_mom_gen_x->GetValue() + ce_mom_gen_y->GetValue()*ce_mom_gen_y->GetValue() +ce_mom_gen_z->GetValue()*ce_mom_gen_z->GetValue()); //l_fit_mom_CE_generated->GetValue();

		histo_CE_generated->Fill(fit_mom_CE_generated);
	}
	Double_t CE_generated_integral = histo_CE_generated->Integral( 1, nBins );
	cout << "CE_generated_integral = " << CE_generated_integral << endl;

	Ngen_CE = histo_CE_generated->GetEntries();
	std::cout << "Ngen_CE == " << Ngen_CE << std::endl;

	// definitions for calculations
	double POT = 3.6E20;
	// double numberStopsDefault = 150940; // 75 TDR beam files
	double numberStopsThis = Ngen_CE; // we create one CE electron per stop
	...
	////////////////////////////////////////////////////////////
	// read in reconstructed CE data and fill histogram
	////////////////////////////////////////////////////////////

	std::cout << std::endl << "Reading reconstructed CE data" << std::endl;

	TFile *file_CE_reconstructed = new TFile(inputfilename_CE_reconstructed.c_str(), "READ");
	TTree *tree_CE_reconstructed = (TTree*) file_CE_reconstructed->Get("TrkAnaNeg/trkana");

	// loop over CE tree and fill histogram
	TBranch *b_fit_CE_reconstructed = tree_CE_reconstructed->GetBranch("deent");
	TLeaf *l_fit_mom_CE_reconstructed = b_fit_CE_reconstructed->GetLeaf("mom");

	for( Long64_t i = 0; tree_CE_reconstructed->GetEntry(i) > 0; i++ ) {
		Float_t fit_mom_CE_reconstructed = l_fit_mom_CE_reconstructed->GetValue();

		histo_CE_reconstructed->Fill(fit_mom_CE_reconstructed);
	}
	Double_t CE_reconstructed_integral = histo_CE_reconstructed->Integral( 1, nBins );
	cout << "CE_reconstructed_integral = " << CE_reconstructed_integral << endl;

	////////////////////////////////////////////////////////////
	// read in generated DIO data and fill histogram
	////////////////////////////////////////////////////////////

	std::cout << std::endl << "Reading generated DIO data" << std::endl;

	TFile *file_DIO_generated = new TFile(inputfilename_DIO_generated.c_str(), "READ");
	TTree *tree_DIO_generated = (TTree*) file_DIO_generated->Get("TrkAnaNeg/trkana");

	// loop over DIO tree and fill histogram
	TBranch *b_fit_DIO_generated_tree_DIO_generated = tree_DIO_generated->GetBranch("demcgen");
	TLeaf *l_fit_mom_DIO_generated_tree_DIO_generated_x = b_fit_DIO_generated_tree_DIO_generated->GetLeaf("momx");
	TLeaf *l_fit_mom_DIO_generated_tree_DIO_generated_y = b_fit_DIO_generated_tree_DIO_generated->GetLeaf("momy");
	TLeaf *l_fit_mom_DIO_generated_tree_DIO_generated_z = b_fit_DIO_generated_tree_DIO_generated->GetLeaf("momz");

	for( Long64_t i = 0; tree_DIO_generated->GetEntry(i) > 0; i++ ) {
	  Float_t dio_mom_gen_gen_x = l_fit_mom_DIO_generated_tree_DIO_generated_x->GetValue();
	  Float_t dio_mom_gen_gen_y = l_fit_mom_DIO_generated_tree_DIO_generated_y->GetValue();
	  Float_t dio_mom_gen_gen_z = l_fit_mom_DIO_generated_tree_DIO_generated_z->GetValue();
		Float_t fit_mom_DIO_generated = sqrt(dio_mom_gen_gen_x*dio_mom_gen_gen_x + dio_mom_gen_gen_y*dio_mom_gen_gen_y + dio_mom_gen_gen_z*dio_mom_gen_gen_z);//l_fit_mom_DIO_generated_tree_DIO_generated->GetValue();

		Double_t dio_weight =  calculate_DIOweight(fit_mom_DIO_generated);

		histo_DIO_generated_flat->Fill(fit_mom_DIO_generated);
		histo_DIO_generated_reweighted->Fill(fit_mom_DIO_generated, dio_weight / 7.91001e-10 ); // 7.91001e-10 is the dio_cz integral from 90 to 105
	}
	Double_t DIO_generated_flat_integral = histo_DIO_generated_flat->Integral( 1, nBins );
	Double_t DIO_generated_reweighted_integral = histo_DIO_generated_reweighted->Integral( 1, nBins );
	cout << "DIO_generated_flat_integral = " << DIO_generated_flat_integral << endl;
	cout << "DIO_generated_reweighted_integral = " << DIO_generated_reweighted_integral << endl;

	////////////////////////////////////////////////////////////
	// read in reconstructed DIO data and fill histogram
	////////////////////////////////////////////////////////////

	std::cout << std::endl << "Reading reconstructed DIO data" << std::endl;

	TFile *file_DIO_reconstructed = new TFile(inputfilename_DIO_reconstructed.c_str(), "READ");
	TTree *tree_DIO_reconstructed = (TTree*) file_DIO_reconstructed->Get("TrkAnaNeg/trkana");

	// loop over CE tree and fill histogram
	TBranch *b_fit_DIO_reconstructed_tree_DIO_reconstructed = tree_DIO_reconstructed->GetBranch("deent");
	TLeaf *l_fit_mom_DIO_reconstructed_tree_DIO_reconstructed = b_fit_DIO_reconstructed_tree_DIO_reconstructed->GetLeaf("mom");

	TBranch *b_fit_DIO_generated_tree_DIO_reconstructed = tree_DIO_reconstructed->GetBranch("demcgen");
	TLeaf *l_fit_mom_DIO_generated_tree_DIO_reconstructed_x = b_fit_DIO_generated_tree_DIO_reconstructed->GetLeaf("momx");
  TLeaf *l_fit_mom_DIO_generated_tree_DIO_reconstructed_y = b_fit_DIO_generated_tree_DIO_reconstructed->GetLeaf("momy");
  TLeaf *l_fit_mom_DIO_generated_tree_DIO_reconstructed_z = b_fit_DIO_generated_tree_DIO_reconstructed->GetLeaf("momz");

	for( Long64_t i = 0; tree_DIO_reconstructed->GetEntry(i) > 0; i++ ) {
		Float_t fit_mom_DIO_reconstructed = l_fit_mom_DIO_reconstructed_tree_DIO_reconstructed->GetValue();
	  Float_t dio_mom_gen_rec_x = l_fit_mom_DIO_generated_tree_DIO_reconstructed_x->GetValue();
	  Float_t dio_mom_gen_rec_y = l_fit_mom_DIO_generated_tree_DIO_reconstructed_y->GetValue();
	  Float_t dio_mom_gen_rec_z = l_fit_mom_DIO_generated_tree_DIO_reconstructed_z->GetValue();
		Float_t fit_mom_DIO_generated = sqrt(dio_mom_gen_rec_x*dio_mom_gen_rec_x + dio_mom_gen_rec_y*dio_mom_gen_rec_y + dio_mom_gen_rec_z*dio_mom_gen_rec_z);//l_fit_mom_DIO_generated_tree_DIO_reconstructed->GetValue();

//		Double_t dio_weight =  calculate_DIOweight(fit_mom_DIO_reconstructed);
		Double_t dio_weight =  calculate_DIOweight(fit_mom_DIO_generated);

		histo_DIO_reconstructed_flat->Fill(fit_mom_DIO_reconstructed);
		histo_DIO_reconstructed_reweighted->Fill(fit_mom_DIO_reconstructed, dio_weight / 7.91001e-10 ); // 7.91001e-10 is the dio_cz integral from 90 to 105
	}
	Double_t DIO_reconstructed_flat_integral = histo_DIO_reconstructed_flat->Integral( 1, nBins );
	Double_t DIO_reconstructed_reweighted_integral = histo_DIO_reconstructed_reweighted->Integral( 1, nBins );
	cout << "DIO_reconstructed_flat_integral = " << DIO_reconstructed_flat_integral << endl;
	cout << "DIO_reconstructed_reweighted_integral = " << DIO_reconstructed_reweighted_integral << endl;

	std::vector<results_struct> results_vector;

	// loop over all possible lower and upper momentum ranges
	//	for (double mom_low=102.5; mom_low<106.5; mom_low+=0.05) {
	//    		for (double mom_high=mom_low+0.05; mom_high<107.; mom_high+=0.05) {
	for (double mom_low=momentum_lower_limit; mom_low<momentum_upper_limit; mom_low+=momentum_Bin_width) {
...
//			cout << "results.BF_UL = " << results.BF_UL << endl;
	...
	int temp_index=-1;
	double temp_BF_UL=999;
	for(std::vector<results_struct>::iterator iter0=results_vector.begin(); iter0!=results_vector.end(); iter0++)
	{
		if ( (*iter0).BF_UL < temp_BF_UL ) {
			temp_index = iter0 - results_vector.begin();
			temp_BF_UL = (*iter0).BF_UL;
		}

		/*
		cout << (*iter0).BF_UL << endl;
		cout <<  iter0 - results_vector.begin()<< endl;
		*/
	}
	results_vector[temp_index].optimal_window=1; // set flag to 1 for the entry with the optimal window

	cout << endl << "At index " << temp_index << " optimal BF upper limit of " << temp_BF_UL << " found." << endl << endl;

	printf(" p_low  p_high  Ngen_CE  Nrec_CE             eff_CE        Nexp_CE                    SES               Ngen_DIO               Nrec_DIO            eff_DIO         Nexp_DIO         Nexp_RPC         NNsig_UL                  BF_UL\n");
	printf("====================================================================================================================================================================================================================================\n");
	printf("%5.2f  %5.2f %8.0f %8.0f  %5.4f +/- %5.4f  %3.2f +/- %3.2f  %3.2E +/- %3.2E  %8.6f +/- %8.6f  %8.6f +/- %8.6f  %5.4f +/- %5.4f  %4.3f +/- %4.3f  %4.3f +/- %4.3f  %4.3f +/- %4.3f  %3.2E +/- %3.2E\n",
			(results_vector[temp_index]).momentum_low,
			(results_vector[temp_index]).momentum_high,
			(results_vector[temp_index]).N_CE_gen,
			(results_vector[temp_index]).N_CE_rec,
			(results_vector[temp_index]).efficiency_CE,
			(results_vector[temp_index]).efficiency_error_CE,
			(results_vector[temp_index]).N_CE_expected,
			(results_vector[temp_index]).N_CE_expected_error,
			(results_vector[temp_index]).SES,
			(results_vector[temp_index]).SES_error,
			(results_vector[temp_index]).N_DIO_gen,
			(results_vector[temp_index]).N_DIO_gen_error,
			(results_vector[temp_index]).N_DIO_rec,
			(results_vector[temp_index]).N_DIO_rec_error,
			(results_vector[temp_index]).efficiency_DIO,
			(results_vector[temp_index]).efficiency_error_DIO,
			(results_vector[temp_index]).N_DIO_expected,
			(results_vector[temp_index]).N_DIO_expected_error,
			(results_vector[temp_index]).N_RPCs_expected,
			(results_vector[temp_index]).N_RPCs_expected_error,
			(results_vector[temp_index]).Nsig_UL,
			(results_vector[temp_index]).Nsig_UL_error,
			(results_vector[temp_index]).BF_UL,
			(results_vector[temp_index]).BF_UL_error );

	// save results to root file
	TFile *filenew = new TFile("data_output_CalTool_reweighting.root","RECREATE");
	TTree *treenew = new TTree("h1","h1");

	// write histograms
	histo_RPCs->Write("histo_RPCs");
	histo_CE_reconstructed->Write();
	histo_CE_generated->Write();
	histo_DIO_reconstructed_flat->Write();
	histo_DIO_generated_flat->Write();
	histo_DIO_reconstructed_reweighted->Write();
	histo_DIO_generated_reweighted->Write();

	// rescale background histograms to final luminosity
//	TH1F *histo_total_background_DIO_RPC_rescaled_to_final_luminosity; // DIO + RPC background

	double integral_DIO_reconstructed_reweighted = histo_DIO_reconstructed_reweighted->Integral( histo_DIO_reconstructed_reweighted->FindBin( (results_vector[temp_index]).momentum_low), histo_DIO_reconstructed_reweighted->FindBin((results_vector[temp_index]).momentum_high) );

	double rescale_factor_DIO_background_to_final_luminosity = (results_vector[temp_index]).N_DIO_expected / integral_DIO_reconstructed_reweighted;

	std::cout << "integral_DIO_reconstructed_reweighted == " << integral_DIO_reconstructed_reweighted << std::endl;
	std::cout << "rescale_factor_DIO_background_to_final_luminosity == " << rescale_factor_DIO_background_to_final_luminosity << std::endl;

	TH1F *histo_DIO_reconstructed_reweighted_rescaled_to_final_luminosity(histo_DIO_reconstructed_reweighted);

	histo_DIO_reconstructed_reweighted_rescaled_to_final_luminosity->Scale(rescale_factor_DIO_background_to_final_luminosity);
	histo_DIO_reconstructed_reweighted_rescaled_to_final_luminosity->Write("histo_DIO_reconstructed_reweighted_rescaled_to_final_luminosity");

	// sum RPCs to the DIO background
	TH1F *histo_total_background_DIO_RPC_rescaled_to_final_luminosity(histo_DIO_reconstructed_reweighted_rescaled_to_final_luminosity);

	histo_total_background_DIO_RPC_rescaled_to_final_luminosity->Add(histo_RPCs);

//	histo_DIO_reconstructed_reweighted_rescaled_to_final_luminosity->Write("histo_DIO_reconstructed_reweighted_rescaled_to_final_luminosity");
	histo_total_background_DIO_RPC_rescaled_to_final_luminosity->Write("histo_total_background_DIO_RPC_rescaled_to_final_luminosity");

	// write data
	results_struct temp_data;

	TBranch *b_momentum_low = treenew->Branch("momentum_low", &temp_data.momentum_low, "momentum_low/D");
	TBranch *b_momentum_high = treenew->Branch("momentum_high", &temp_data.momentum_high, "momentum_high/D");
	TBranch *b_N_CE_gen = treenew->Branch("N_CE_gen", &temp_data.N_CE_gen, "N_CE_gen/D");
	TBranch *b_N_CE_rec = treenew->Branch("N_CE_rec", &temp_data.N_CE_rec, "N_CE_rec/D");
	TBranch *b_efficiency_CE = treenew->Branch("efficiency_CE", &temp_data.efficiency_CE, "efficiency_CE/D");
	TBranch *b_efficiency_error_CE = treenew->Branch("efficiency_error_CE", &temp_data.efficiency_error_CE, "efficiency_error_CE/D");
	TBranch *b_N_CE_expected = treenew->Branch("N_CE_expected", &temp_data.N_CE_expected, "N_CE_expected/D");
	TBranch *b_N_CE_expected_error = treenew->Branch("N_CE_expected_error", &temp_data.N_CE_expected_error, "N_CE_expected_error/D");
	TBranch *b_SES = treenew->Branch("SES", &temp_data.SES, "SES/D");
	TBranch *b_SES_error = treenew->Branch("SES_error", &temp_data.SES_error, "SES_error/D");
	TBranch *b_N_DIO_gen = treenew->Branch("N_DIO_gen", &temp_data.N_DIO_gen, "N_DIO_gen/D");
	TBranch *b_N_DIO_gen_error = treenew->Branch("N_DIO_gen_error", &temp_data.N_DIO_gen_error, "N_DIO_gen_error/D");
	TBranch *b_N_DIO_rec = treenew->Branch("N_DIO_rec", &temp_data.N_DIO_rec, "N_DIO_rec/D");
	TBranch *b_N_DIO_rec_error = treenew->Branch("N_DIO_rec_error", &temp_data.N_DIO_rec_error, "N_DIO_rec_error/D");
	TBranch *b_efficiency_DIO = treenew->Branch("efficiency_DIO", &temp_data.efficiency_DIO, "efficiency_DIO/D");
	TBranch *b_efficiency_error_DIO = treenew->Branch("efficiency_error_DIO", &temp_data.efficiency_error_DIO, "efficiency_error_DIO/D");
	TBranch *b_N_DIO_expected = treenew->Branch("N_DIO_expected", &temp_data.N_DIO_expected, "N_DIO_expected/D");
	TBranch *b_N_DIO_expected_error = treenew->Branch("N_DIO_expected_error", &temp_data.N_DIO_expected_error, "N_DIO_expected_error/D");
	TBranch *b_N_RPCs_expected = treenew->Branch("N_RPCs_expected", &temp_data.N_RPCs_expected, "N_RPCs_expected/D");
	TBranch *b_N_RPCs_expected_error = treenew->Branch("N_RPCs_expected_error", &temp_data.N_RPCs_expected_error, "N_RPCs_expected_error/D");
	TBranch *b_Nsig_UL_error = treenew->Branch("Nsig_UL_error", &temp_data.Nsig_UL_error, "Nsig_UL_error/D");
	TBranch *b_BF_UL = treenew->Branch("BF_UL", &temp_data.BF_UL, "BF_UL/D");
	TBranch *b_BF_UL_error = treenew->Branch("BF_UL_error", &temp_data.BF_UL_error, "BF_UL_error/D");
	TBranch *b_flag_optimal_window = treenew->Branch("flag_optimal_window", &temp_data.optimal_window, "flag_optimal_window/I");

	// loop over results_vector and fill tree
	for(std::vector<results_struct>::iterator iter0=results_vector.begin(); iter0!=results_vector.end(); iter0++)
	{
		temp_data = (*iter0);
